package conscape.util
{
	...
	public class FlareMaths
	{
		...
		public static function logFloor(x:Number, base:Number):Number {
			if (x > 0) {
				return Math.pow(base, Math.floor(FlareMaths.log(x, base)));
			} else {
				return -Math.pow(base, -Math.floor(-FlareMaths.log(-x, base)));
			}
		}
		...
		public static function logCeil(x:Number, base:Number):Number {
			if (x > 0) {
				return Math.pow(base, Math.ceil(FlareMaths.log(x, base)));
			} else {
				return -Math.pow(base, -Math.ceil(-FlareMaths.log(-x, base)));
			}
		}
