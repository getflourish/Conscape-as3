package com.modestmaps.overlays
{
	...
	public class MarkerClip extends TouchSprite
	{
	    ...
	    public function MarkerClip(map:Map)
	    {
			...
			addEventListener(TouchEvent.TOUCH_TAP, onMarkerTap);
	    ...
	    */
        ...
        protected function onMarkerTap (event:TouchEvent):void
        {
            var marker:DisplayObject = event.target as DisplayObject;
            var location:Location = getMarkerLocation( marker );
            dispatchEvent(new MarkerEvent(MarkerEvent.MARKER_TAP, marker, location, true) );
        }	
	}
